...
| `excel_range` | `read` (paginated, 100-row default; spill metadata in response), `read_spill` (full spill range for dynamic array anchor), `write`, `write_formula` (Formula2/dynamic arrays), `clear` |
...
- **Phase 4:** Charts (`excel_chart`) + `excel_screenshot` (visual verification loop) + `=PY()` write-only
