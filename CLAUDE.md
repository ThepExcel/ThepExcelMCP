...
uv run pytest -q                                                 # 1000+ unit tests (no Excel needed)
...
THEPEXCEL_MCP_AUTOLAUNCH=1 uv run python tests/bench_com.py     # live perf bench (µs/op to stderr; not pytest-collected)
...
- **Perf layer (2026-07-08):** `get_app()` caches the Application handle (liveness probe =
  `.Name`; any exception drops the cache and re-resolves). Paginated reads (`excel_range read/
  read_spill`, `excel_table read`, `excel_pivot read`) marshal only the page's `Cells(...)`-bounded
  sub-range — never whole-range `.Value` + Python slice. `bulk_guard(app)` (session.py) suppresses
  ScreenUpdating/EnableEvents + manual-calc around bulk write loops (table append_rows, border
  all/inside, find_replace replace, load_to_table sheet-rebuild) — NEVER around PQ refresh /
  datamodel / cube paths (deadlock + async-calc semantics). Slow COM calls
  (> `THEPEXCEL_MCP_SLOW_LOG_S`, default 5.0s) log one line to stderr.
  `THEPEXCEL_MCP_EARLYBIND=1` (default ON) re-wraps the SAME attached IDispatch
  pointer with a generated wrapper; a falsy value forces a truly dynamic wrapper.
  Read actions on range/table/pivot accept `value_mode="raw"` for faster `.Value2`
  serial values while the compatibility default remains typed `.Value`. Table append
  uses one `ListObject.Resize` only when the expansion band is empty, otherwise it
  falls back to insertion-preserving `ListRows.Add` and verifies the final row count.
...
- **None outstanding.** All planned phases (0–5) plus Tier-3/4/4b batches have shipped: packaging (`scripts/build_mcpb.py` → `dist/thepexcel-mcp.mcpb`), client registration (`claude mcp add`, see Dev commands above), live end-to-end smoke vs real Excel, and opt-in BM25 progressive discovery (`THEPEXCEL_MCP_TOOL_DISCOVERY=bm25`).
...
- **2026-06-24 — Live-MCP-SURFACE test procedure + lingering-Excel gotcha (all 24 tools verified through the live MCP this session, not just smoke_com.py).** To test THROUGH the MCP surface: (a) the `thepexcel-excel` server must run the latest code — restart after edits (editable ≠ hot reload); (b) Excel must be open — ~~this server has **NO AUTOLAUNCH**~~ **[SUPERSEDED 2026-07-06: auto-launch is now ON by default + adds a blank workbook + self-heals the gen_py cache, so `get_app()` opens Excel itself; the manual launch below is no longer required.]** When Excel is closed (and you've opted out via `THEPEXCEL_MCP_AUTOLAUNCH=0`), launch a throwaway instance: `uv run python` → `win32com.client.DispatchEx("Excel.Application")`, `.Visible=True`, **`.Workbooks.Add()`** (the Add is REQUIRED — Excel registers in the ROT only once it owns a document, so otherwise `GetActiveObject` can't find it). GOTCHA: after the test, the launched `EXCEL.EXE` can **linger a short while after `app.Quit()`** because the MCP server's STA worker still holds a COM reference — it self-clears within ~a minute (or `taskkill //PID <pid>`). **[RESOLVED 2026-07-24: `tests/smoke_com.py` + `tests/bench_com.py` now self-clean via `tests/_excel_lifecycle.py::ExcelInstanceGuard` — snapshots EXCEL.EXE PIDs before the first `get_app()`, quits ONLY a PID the harness itself spawned, and re-checks the live PID immediately before Close/Quit so a stale cache re-attaching to the user's real Excel can never trigger a wrong-instance quit. A pre-existing Excel is never touched.]** SAFETY: `get_app()` uses `GetActiveObject` = the FIRST ROT Excel instance; if the user has THEIR OWN Excel open, a `workbook=None` call targets THEIR active workbook — when testing, always create + target an explicit test workbook (e.g. `Book1`) and never assume the active workbook is disposable. SELF-CORRECTION (this session): a 538 MB `EXCEL.EXE` was MY test instance's own memory footprint, NOT a user workbook — do NOT anchor on a stale "user has the 536 MB workbook open" note from a prior session; verify against live `tasklist` / `excel_workbook(list)` before claiming whose process it is.
...
- **2026-07-08 — Performance pass (12 optimizations, 927→1008 tests, live smoke targeted sections PASS; two independent analyses converged on the same top-4).** Facts that must survive future edits: (1) **Pagination was quadratic** — `_read` paths marshaled the ENTIRE range via `.Value` then Python-sliced the page; now they read only the page's `ws.Range(ws.Cells(r1+offset,c1), ws.Cells(last,cN))` block. Preserve the legacy quirk: a truly-empty 1×1 range reports `total_rows=0` (not one empty row). (2) **`append_rows`** = ListRows.Add per row + ONE block `.Value` write capped at the WIDEST SUPPLIED row (not full table width) — writing None across unsupplied columns would clear calculated-column formulas that ListRows.Add auto-fills. (3) **`bulk_guard`** restores the SAVED Calculation mode (user may run manual calc deliberately) and forces one `Calculate()` only if it changed the mode; never wrap PQ-refresh/datamodel/cube. (4) **App cache**: `_cached_app` lives on the STA worker only; `.Name` probe; any exception → drop + re-resolve through the unchanged attach/self-heal/auto-launch chain. (5) **Early binding** (`THEPEXCEL_MCP_EARLYBIND=1`, default OFF): gencache wrapper adopted only on `.Hwnd` match — EnsureDispatch does NOT attach like GetActiveObject and can spawn a new Excel process. Flag-on passed smoke sections 1,2,3,4,11,26,27 (48/48). **[SUPERSEDED 2026-07-24 — see the perf-round-2 entry below: this Hwnd-match/EnsureDispatch construction was replaced entirely, and the flag is now default-ON.]** (6) **Spill fallback scan** is exponential+binary search over `HasSpill` (~20 COM calls vs ~1000); primary path is still `SpillingRange`. (7) **Pre-existing smoke failures** (NOT regressions; reproduced 3/3 on unmodified code): Data Model relationship/measure "table not found" cascade in §5/8 + `cube.live_cube_value` §18 — same headless-stdio datamodel limitation family. (8) Slow-call diagnostics: worker logs calls > `THEPEXCEL_MCP_SLOW_LOG_S` (default 5.0s) to stderr — stdout is the stdio transport, never print there.
- **2026-07-24 — Perf round 2: `THEPEXCEL_MCP_EARLYBIND` flipped to default ON (kill-switch semantics), zero-activation rewrap replaces the old EnsureDispatch/Hwnd-match construction.** `_maybe_earlybind` now reads the typelib off the already-attached late-bound Application (`_oleobj_.GetTypeInfo()`→`GetContainingTypeLib()`→`GetLibAttr()`) and re-wraps the SAME IDispatch pointer early-bound via `gencache.EnsureModule`+`win32com.client.Dispatch` — never activates COM, so it cannot spawn a duplicate Excel (unlike the old `EnsureDispatch`). Flip decided on: bench `tests/bench_com.py` showed ~1.25x faster property reads (709µs late vs 568µs early, median of 5×1000 reads on this machine) and a full `smoke_com.py` §1-28 run (161 checks) was **byte-identical pass/fail between late- and early-bound** — same 8 pre-existing datamodel/cube failures (§5 relationship/measure cascade + §18 `cube.live_cube_value`), zero new regressions. Also grepped all `domains/*.py` for bool/tristate comparisons that early binding's typed returns could break (`== -1`, `is True`/`is False` on COM reads, `int()`-wrapped COM booleans) — clean: every boolean COM read-back in this codebase uses truthiness (`if x:`/`if not x:`), which is representation-agnostic; the only literal `-1`/`0` comparisons found are doc-prose, write-only MsoTriState constants (`shapes.py` `_MSO_TRUE`/`_MSO_FALSE`, assigned TO COM properties, never read back), or pure-Python parameter sentinels unrelated to COM. Set `THEPEXCEL_MCP_EARLYBIND=0` to force late binding if a future Office build's early-bound typelib misbehaves. P3 (`.Value2` opt-in lane) was explicitly skipped per decision — not shipped this round.
- **2026-08-04 — Optimization correction + reproducibility pass (supersedes the 2026-07-24 benchmark magnitude and Value2 omission).** (1) A wrapper-forced live bench around the SAME IDispatch pointer measured early-bound property reads at 244µs/op vs dynamic 265µs/op (1.09x, not the earlier claimed 1.25x); the kill-switch now explicitly constructs a dynamic wrapper because `GetActiveObject` can return `gen_py` even when rewrap is skipped. (2) Raw `value_mode="raw"` shipped for range/table/pivot reads: latest 100k-cell mixed-data bench `.Value2` 91.94ms vs `.Value` 112.15ms (1.22x); typed `.Value` remains default because date/currency semantics differ. (3) `append_rows` now uses one safe `ListObject.Resize` when the full expansion band is empty, with insertion-preserving `ListRows.Add` fallback when occupied; latest tool-entry benchmark = 1,000 rows in 165ms (~6,054 rows/s) with calculated-column read-back, versus the 7.15s per-row baseline (~43x). (4) Tool discovery is opt-in BM25: catalog measured ~59k→3.6k chars (~94% smaller), while the full 26-tool default stays compatible. (5) Packaging now requires committed `uv.lock`; CI uses `uv sync --frozen`, lint, 1,043 unit tests, an isolated MCP SDK 2.0.0 stdio handshake (26 full / 4 discovery tools, negotiated 2025-11-25), and bundle build. Full live smoke = 154 PASS / 8 known pre-existing Data Model+CUBE FAIL / 2 capability SKIP; all newly changed live read-back checks passed.

> Absorbed FableLegacy runbooks (2026-07-31) live in `.claude/skills/` — any skill whose banner says ⚠️ PARTIAL needs its flagged sections re-verified before use (audit: D:/claude-master/research/2026-07-31-fablelegacy-absorption/).
