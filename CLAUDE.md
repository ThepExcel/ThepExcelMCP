...
| File | Role |
|---|---|
| `src/thepexcel_mcp/server.py` | FastMCP app + 23 tool registrations |
...
uv run pytest -q                                                 # 893 unit tests (no Excel needed)
THEPEXCEL_MCP_AUTOLAUNCH=1 uv run python tests/smoke_com.py     # full live COM smoke
THEPEXCEL_MCP_AUTOLAUNCH=1 uv run python tests/smoke_com.py --sections 1,2,3,4  # partial (sections 1-27 available)
