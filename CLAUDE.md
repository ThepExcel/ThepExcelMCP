...
uv run pytest -q                                                 # 1008 unit tests (no Excel needed)
...
- **Perf layer (2026-07-08):** `get_app()` caches the Application handle (liveness probe =
  ...
  `THEPEXCEL_MCP_EARLYBIND=1` (experimental, default OFF) swaps in a gencache early-bound
  Application wrapper only when its `.Hwnd` matches the attached instance.
